...
1. Here we are only creating the Makejail that defines what our jail will be. The common options are `start` and `overwrite=force` used by `appjail-quick(1)`, but we also specify a volume called `badwolf-data` so that if we want to download something, it will be kept even if we recreate the jail by calling the `create.sh` script again, but please note that creating a volume does not mount it, so you must also specify the `fstab` option. You can also specify network options, but I recommend doing so outside of the Makejail, as this will allow you to customize them more easily and efficiently, as we do in `create.sh`.
